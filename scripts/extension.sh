...
function deploy() {
    ...
    if ! [ "$(grep 'failed=' /var/log/deploy/deploy.log | awk '{print $6}' | awk -F '=' '{print $2}' | awk '$1 != 0')" = "" ] ; then
      webhook_process "ready_environment" "准备扩容环境失败" "false" 1 "准备扩容环境"
      exit 1
    fi
    ...
    if [ "$(grep 'failed=' /var/log/deploy/deploy.log | awk '{print $6}' | awk -F '=' '{print $2}' | awk '$1 != 0')" = "" ] ; then
      webhook_process "ready_environment" "成功" "true" 1 "准备扩容环境"
    else
      webhook_process "ready_environment" "准备扩容环境失败" false 1 "准备扩容环境"
      exit 1
    fi
    ...
    if [ "$deploy_ceph_flag" = "True" ]; then
      ...
      if [ "$(grep 'failed=' /var/log/deploy/deploy.log | awk '{print $6}' | awk -F '=' '{print $2}' | awk '$1 != 0')" = "" ] ; then
        webhook_process "deploy_ceph" "成功" "true" 2 "扩容文件系统"
      else
        webhook_process "deploy_ceph" "扩容文件系统失败" "false" 2 "扩容文件系统"
        exit 1
      fi
    else
      webhook_process "deploy_ceph" "成功" "true" 2 "扩容文件系统"
    ...
    if [ "$(grep 'failed=' /var/log/deploy/deploy.log | awk '{print $6}' | awk -F '=' '{print $2}' | awk '$1 != 0')" = "" ] ; then
      webhook_process "deploy_trochilus" "成功" "true" 3 "扩容虚拟化系统"
    else
      webhook_process "deploy_trochilus" "扩容虚拟化系统失败" false 3 "扩容虚拟化系统"
      exit 1
    fi
    exit 0
}
